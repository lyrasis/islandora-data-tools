...
case 'CdmCompound':
    cdm_compound_writer($record_key, $path_to_schema, $info_log, $error_log, $out_dir);
    break;
case 'CdmNewspapers':
    cdm_newspapers_writer($record_key, $children_record_keys, $path_to_schema, $mods_filename, $item_info_field_for_issues, $config, $info_log, $error_log, $out_dir);
    break;
case 'CsvSingleFile':
    csv_single_file_writer($record_key, $path_to_schema, $config, $info_log, $error_log, $out_dir);
    break;
default:
    cdm_single_file_writer($record_key, $path_to_schema, $config, $info_log, $error_log, $out_dir);
    break;
}
 ...
 *   A Monolog logger object.
 */
function cdm_single_file_writer($record_key, $path_to_schema, $config, $info_log, $error_log, $out_dir) {
    $path_to_mods = $out_dir . DIRECTORY_SEPARATOR .
                  $record_key . '.xml';
    $info_log->addInfo("Running cdm_single_file_writer_validation", array('MODS file' => $path_to_mods));
    validate_mods($path_to_schema, $path_to_mods, $info_log, $error_log);
}

function cdm_compound_writer($record_key, $path_to_schema, $info_log, $error_log, $out_dir) {
    $obj_dir = $out_dir . DIRECTORY_SEPARATOR . $record_key;
    $path_to_mods = $obj_dir . DIRECTORY_SEPARATOR . 'MODS.xml';
    $info_log->addInfo("Running cdm_compound_writer_validation on parent object", array('MODS file' => $path_to_mods));
    validate_mods($path_to_schema, $path_to_mods, $info_log, $error_log);
    ...
    foreach($child_dirs as $cd){
        ...
        $info_log->addInfo("Running cdm_compound_writer_validation on child object", array('MODS file' => $path_to_mods));
        validate_mods($path_to_schema, $path_to_mods, $info_log, $error_log);
    }
}
 ...
 *   A Monolog logger object.
 */
function cdm_newspapers_writer($record_key, $children_record_keys, $path_to_schema, $mods_filename, $item_info_field_for_issues, $config, $info_log, $error_log, $out_dir) {
    $issue_dir = get_issue_dir($record_key, $item_info_field_for_issues, $config);
    $dir = $out_dir . DIRECTORY_SEPARATOR . $issue_dir;
    ...
    foreach ($directory_iterator as $filepath => $info) {
        if (preg_match('/' . $mods_filename . "$/", $filepath)) {
            $info_log->addInfo("Running cdm_newspapers_writer validation", array('MODS file' => $path_to_mods));
 ...
 *   A Monolog logger object.
 */
function csv_single_file_writer($record_key, $path_to_schema, $config, $info_log, $error_log, $out_dir) {
    if (isset($config['WRITER']['preserve_content_filenames']) && $config['WRITER']['preserve_content_filenames']) {
        ...
        $path_to_mods = $out_dir . DIRECTORY_SEPARATOR .
                      $filename . '.xml';
        $info_log->addInfo("Running csv_single_file_writer validation", array('MODS file' => $path_to_mods));
        validate_mods($path_to_schema, $path_to_mods, $info_log, $error_log);
    }
    else {
        $path_to_mods = $out_dir . DIRECTORY_SEPARATOR .
